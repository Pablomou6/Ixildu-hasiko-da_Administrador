...
public class DAOTrabajadores extends AbstractDAO {
    ...
    public Boolean insertarTrabajador(Trabajador trab, String tipo, ArrayList<Integer> salasSelec) {
        ...
        try {
            ...
            try(PreparedStatement stmtTrabajar = con.prepareStatement(sqlTrabajar)){
    ...
    public List<Trabajador> obtenerTrabajador(String nombre, String dni) {
        List<Trabajador> resultado = new ArrayList<>();
    String consulta = "SELECT * FROM trabajadorproyeccion WHERE nombre LIKE '%"+ nombre + "%' OR dni LIKE '%"+ dni + "%' " +
                      "UNION " +
                      "SELECT * FROM trabajadorlimpieza WHERE nombre LIKE '%"+ nombre + "%' OR dni LIKE '%"+ dni + "%'";

    try (Connection con = this.getConexion();
         PreparedStatement stmTrabajadores = con.prepareStatement(consulta)) {
        
        System.out.println("Consulta: " + consulta);
        
        try (ResultSet rsTrabajadores = stmTrabajadores.executeQuery()) {
